---
title: "Threaded Comments & Mentions in MC Modeler"
sidebarTitle: "Comments"
description: "Add threaded comments to diagram elements, @mention collaborators, receive in-app and email notifications, and deep-link directly to comment threads."
---
...
With the Comments panel open (`Ctrl+Shift+C` or the toolbar toggle), click any element on the canvas while comments mode is active. The **SelectionCommentTrigger** detects the click and opens the `NewThreadComposer` inline, pre-populated with the element's label as the anchor.

Type your comment and press **Ctrl+Enter** (or click **Comment**) to create the thread. The thread is attached to the element via an `Anchor` object that stores the element ID and label:
